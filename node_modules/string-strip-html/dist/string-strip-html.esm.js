...
function stripHtml(str, originalOpts) {
    ...
    function treatRangedTags(i, opts, rangesToDelete) {
            ...
            if (tag.slashPresent &&
                ...
                for (let y = rangedOpeningTagsForDeletion.length; y--;) {
                    if (rangedOpeningTagsForDeletion[y].name === tag.name) {
                        filteredTagLocations = filteredTagLocations.filter(([from, upto]) => (from < rangedOpeningTagsForDeletion[y].lastOpeningBracketAt ||
                            from >= i + 1) &&
        ...
        else if (Array.isArray(opts.ignoreTagsWithTheirContents) &&
                ...
                for (let y = i; y--;) {
                    if (str[y - 1] === undefined || isClosingAt(y)) {
                        ...
                        if (str !== `<${trim(culprit.trim(), "/>")}>` &&
                            [...definitelyTagNames].some((val) => trim(culprit
                                .trim()
                                .split(/\s+/)
